...
object DeepSeekDirect {
    ...
    private const val MAX_ROUNDS = 20000
    ...
    /** 汇总 MCP 工具名：内置 McpService + 外部 MCP 服务器（设置里配置的地址） */
    private fun mcpToolNames(ctx: Context): Set<String> {
        val names = HashSet<String>()
        runCatching {
            names.addAll(McpService.toolNames())
            val ext = mcpListTools(ctx, "")
            if (!ext.startsWith("错误") && !ext.startsWith("MCP 错误") && !ext.startsWith("该 MCP 服务器")) {
                ext.lineSequence().forEach { line ->
                    val idx = line.indexOf(':')
                    if (idx > 0) names.add(line.substring(0, idx).trim())
                }
            }
        }
        return names
    ...
    private fun executeTool(ctx: Context, tc: ToolCall): String {
        // MCP 优先：先从 MCP 工具列表（内置 McpService + 外部 MCP 服务器）查找匹配的工具
        val mcpNames = mcpToolNames(ctx)
        if (tc.name in mcpNames) {
            val args = runCatching { JSONObject(tc.arguments) }.getOrElse { JSONObject() }
            val r = McpService.callTool(ctx, tc.name, args)
            if (!r.startsWith("错误：MCP 服务器没有这个工具")) return r
            return mcpCall(ctx, "", tc.name, args)
        }
        return try {
            when (tc.name) {
                "exec_command" -> {
    ...
    private fun ghApiFull(repo: String, path: String, token: String, method: String, body: String? = null): Pair<Int, String> {
        val url = if (path.isBlank()) "https://api.github.com/repos/$repo"
                  else "https://api.github.com/repos/$repo/$path"
